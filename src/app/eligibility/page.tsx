...
import { recommendScheme, RecommendationResult, recommendSchemes, RecommendationResultMulti } from "@/lib/recommendScheme";
...
export default function EligibilityPage() {
    ...
    const submitEligibility = async () => {
        ...
        try {
            ...
            // Store top eligible schemes as JSON for the database
            const eligibleSchemes = multiResults.filter(s => s.eligible).slice(0, 5).map(s => ({
                id: s.schemeId,
                name: s.schemeName,
                eligibility: s.matchScore,
                approval: s.approvalProbability,
                category: s.category
            }));
            ...
            const { error } = await supabase.from('eligibility_requests').insert({
                ...
                recommended_scheme_id: result.schemeId,
    ...
    const Toggle = ({ active, onChange }: { active: boolean; onChange: (v: boolean) => void }) => (
        <button
            ...
            className={cn(
                "relative inline-flex h-[28px] w-[52px] flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-primary-500 focus:ring-offset-2",
    ...
    const inputClasses = "w-full rounded-xl border border-gray-200 bg-white/50 px-4 py-3.5 text-sm text-gray-900 placeholder:text-gray-400 focus:border-primary-500 focus:bg-white focus:outline-none focus:ring-4 focus:ring-primary-500/10 hover:border-gray-300 hover:bg-white transition-all duration-200";
    const labelClasses = "block text-sm font-semibold text-gray-600 mb-2.5";
    ...
    return (
        <main className="min-h-screen bg-[#f8fafc] pb-24">
            ...
            <div className="mx-auto max-w-4xl px-4 py-16 sm:px-6 lg:px-8">
                {/* Form Container */}
                <div className="bg-white rounded-3xl border border-gray-100 shadow-[0_8px_30px_rgb(0,0,0,0.04)] p-8 md:p-12 min-h-[600px] flex flex-col">

                    {/* Progress Bar (Boxes) */}
                    <div className="flex gap-3 mb-12 overflow-x-auto pb-4 no-scrollbar -mx-4 px-4 md:mx-0 md:px-0">
                        {steps.map((step) => {
                            ...
                            return (
                                <div
                                    key={step.id}
                                    className={cn(
                                        "flex h-12 flex-1 items-center justify-center rounded-xl border px-5 min-w-[70px] transition-all duration-300",
                                        isCompleted ? "border-green-100 bg-green-50/50" : "",
                                        isActive ? "border-primary-200 bg-primary-50/50 ring-4 ring-primary-50" : "",
                                        isPending ? "border-transparent bg-gray-50/80" : ""
                                    )}
                                >
                                    {isCompleted ? (
                                        <div className="flex items-center justify-center rounded bg-green-400 text-white w-6 h-6 shadow-sm">
                                            <Check size={14} strokeWidth={4} />
                                        </div>
                                    ) : (
                                        <span className={cn(
                                            "text-xl transition-all duration-300",
                                            isPending ? "opacity-40 grayscale scale-95" : "scale-110"
                                        )}>
                                            {step.emoji}
                                        </span>
                                    )}
                        ...
                        {submitSuccess && recommendation && multiRecommendations.length > 0 ? (
                            ...
                            >
                                ...
                                {(() => {
                                    ...
                                    return (
                                        <div className={`w-full rounded-2xl border-2 p-8 text-center ${eligibleCount > 0
                                            ? 'border-emerald-200 bg-gradient-to-br from-emerald-50/80 to-teal-50/60'
                                            ...
                                            <div className="flex items-center justify-center gap-3 mt-6">
                                                <button
                                                    ...
                                                    className="inline-flex items-center gap-2 rounded-xl border border-gray-200 bg-white px-5 py-2.5 text-sm font-semibold text-gray-700 hover:bg-gray-50 transition-all active:scale-95"
                                                >
                                                    Rerun Analysis <RefreshCw size={15} />
                                                </button>
                                            </div>
                                        </div>
                                    );
                                ...
                                <div className="w-full rounded-2xl border border-blue-100 bg-blue-50/50 p-5">
                                    <div className="flex items-start gap-3">
                                        <div className="mt-0.5 flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-blue-100 text-blue-600">
                                            <Info size={18} />
                                        </div>
                                        <div>
                                            <h3 className="text-sm font-bold text-gray-900 mb-1">How to Use These Results</h3>
                                            <p className="text-xs text-gray-600 leading-relaxed">
                                                Schemes marked <strong className="text-emerald-600">✓ Eligible</strong> meet all hard criteria based on your profile.
                                                Schemes marked <strong className="text-red-500">✗ Not Eligible</strong> have specific disqualification reasons shown when expanded.
                                    ...
                                    {multiRecommendations.map((scheme, index) => {
                                        ...
                                        return (
                                            <motion.div
                                                ...
                                                className={`w-full rounded-2xl border bg-white transition-all duration-200 ${cardBorder}`}
                                            >
                                                        ...
                                                        >
                                                            <div className="px-5 sm:px-6 pb-5 sm:pb-6 pt-2 border-t border-gray-100">
                                                                ...
                                                                <div className="flex items-center gap-2 flex-wrap">
                                                                    ...
                                                                    <button
                                                                        onClick={() => router.push(`/scheme/${scheme.schemeId}`)}
                                                                        className="inline-flex items-center gap-1 rounded-lg bg-gray-900 px-4 py-2 text-xs font-bold text-white hover:bg-black active:scale-95 transition-all"
                                                                    >
                            ...
                            <AnimatePresence mode="wait">
                                ...
                                {currentStep === 0 && (
                                    <motion.div
                                        ...
                                        className="space-y-8"
                                    >
                                        <h2 className="text-2xl font-bold text-gray-900 flex items-center gap-3 border-b border-gray-50 pb-6">
                                            <span className="text-3xl drop-shadow-sm">🏢</span> Business Profile
                                        </h2>
                                        ...
                                        <div className="space-y-6">
                                            ...
                                            <div className="pt-8 space-y-5">
                                                <div className="flex items-center justify-between group rounded-xl p-3 hover:bg-gray-50/50 transition-colors -mx-3">
                                ...
                                {currentStep === 1 && (
                                    ...
                                    >
                                        <h2 className="text-2xl font-bold text-gray-900 flex items-center gap-3 border-b border-gray-50 pb-6">
                                            <span className="text-3xl drop-shadow-sm">💰</span> Financial Details
                                        </h2>
                                ...
                                {currentStep === 2 && (
                                    ...
                                    >
                                        <h2 className="text-2xl font-bold text-gray-900 flex items-center gap-3 border-b border-gray-50 pb-6">
                                            <span className="text-3xl drop-shadow-sm">📊</span> Credit Profile
                                        </h2>
                                        ...
                                        <div className="space-y-10">
                                            <div>
                                                ...
                                                <div className="relative mb-10 px-1">
                                                    ...
                                                    <input
                                                        ...
                                                        className="absolute top-1/2 -translate-y-1/2 w-full h-4 appearance-none bg-transparent outline-none cursor-grab active:cursor-grabbing [&::-webkit-slider-thumb]:appearance-none [&::-webkit-slider-thumb]:w-6 [&::-webkit-slider-thumb]:h-6 [&::-webkit-slider-thumb]:rounded-full [&::-webkit-slider-thumb]:bg-primary-500 [&::-webkit-slider-thumb]:shadow-[0_0_0_8px_rgba(255,255,255,1),0_2px_4px_8px_rgba(0,0,0,0.1)] [&::-webkit-slider-thumb]:transition-transform [&::-webkit-slider-thumb]:hover:scale-110 z-10"
                                                    />
                                                </div>

                                                {/* Slider labels */}
                                                <div className="flex justify-between text-xs font-semibold tracking-wider text-gray-400 mt-4 relative uppercase">
                                                    <span>300</span>
                                                    <span
                                                        className="absolute text-primary-600 font-bold -translate-x-1/2 bg-primary-50 px-3 py-1 rounded-full text-[11px]"
                                            ...
                                            <div className="pt-8 border-t border-gray-100 flex items-center justify-between group rounded-xl p-3 hover:bg-gray-50/50 transition-colors -mx-3">
                                ...
                                {currentStep === 3 && (
                                    ...
                                    >
                                        <h2 className="text-2xl font-bold text-gray-900 flex items-center gap-3 border-b border-gray-50 pb-6">
                                            <span className="text-3xl drop-shadow-sm">🚀</span> Expansion Plan
                                        </h2>
                                        ...
                                        <div className="space-y-8">
                                            ...
                                            <motion.div
                                                ...
                                                animate={{ opacity: 1, y: 0 }}
                                                className="overflow-hidden rounded-2xl bg-gradient-to-r from-blue-50 to-indigo-50 border border-blue-100/50"
                                            >
                                                <div className="p-5 flex gap-4 items-start sm:items-center">
                                                    <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-white shadow-sm text-xl border border-blue-100">
                                                        💡
                                                    </div>
                                                    <p className="text-[15px] text-slate-700 leading-relaxed">
                                                        Requesting <span className="font-bold text-blue-700 mx-1 border-b-2 border-blue-200">{formatLoanAmount(requestedLoan)}</span> — ensure this aligns with your business plan and repayment capacity.
                                                    </p>
                                                </div>
                                            </motion.div>
                                        </div>
                    ...
                    {/* Nav Buttons */}
                    {!submitSuccess && (
                        <div className="mt-8 flex items-center justify-between gap-4 pt-8 border-t border-gray-100 mt-auto">
                            {currentStep > 0 ? (
                                <button
                                    onClick={prevStep}
                                    disabled={isSubmitting}
                                    className="rounded-xl border border-gray-200 bg-white px-8 py-3.5 text-sm font-bold text-gray-600 hover:bg-gray-50 hover:border-gray-300 focus:ring-4 focus:ring-gray-100 active:scale-95 transition-all outline-none disabled:opacity-50 disabled:cursor-not-allowed"
                                >
                                    &larr; Back
                                </button>
                            ) : (
                            ...
                            {currentStep < steps.length - 1 ? (
                                <button
                                    onClick={nextStep}
                                    className="rounded-xl bg-primary-600 px-8 py-3.5 text-sm font-bold text-white shadow-md shadow-primary-500/20 hover:bg-primary-700 focus:ring-4 focus:ring-primary-500/20 active:scale-95 transition-all outline-none"
                                >
                                    Continue &rarr;
                                </button>
                            ) : (
                                <button
                                    ...
                                    disabled={isSubmitting}
                                    className="group flex items-center gap-2 rounded-xl bg-emerald-500 px-8 py-3.5 text-sm font-bold text-white shadow-md shadow-emerald-500/20 hover:bg-emerald-600 hover:shadow-lg focus:ring-4 focus:ring-emerald-500/20 active:scale-95 transition-all outline-none disabled:opacity-70 disabled:cursor-not-allowed"
                                >
                                    {isSubmitting ? (
                                        <>
                                        ...
                                        <>
                                            <span className="transition-transform group-hover:scale-110">🔍</span> Analyze My Eligibility
                                        </>
                                    )}
                                </button>
                            )}
